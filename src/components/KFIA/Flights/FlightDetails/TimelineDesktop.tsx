...
export default function TimelineDesktop({
  BORDER, TILE, presentedSteps, isArrival, open, setOpen, toggleAll, allOpen,
}: {
  BORDER: string; TILE: string;
  presentedSteps: Array<JourneyStep & { __badgeKind?: ArrivalBadgeKind; __badgeValue?: string; __timeLabel?: string }>;
  isArrival: boolean;
  ...
  return (
    <div className="kfia-content px-3 sm:px-4 md:px-0 max-w-[1100px] mx-auto mt-8 hidden md:block">
      <div className={`${TILE}`}>
        ...
        <ol className="relative px-6 pb-6">
          ...
          {presentedSteps.map((step) => {
            const isWalk = step.id === "walk-to-security" || step.id === "walk-to-gate" || step.id === "arrival-walk";
